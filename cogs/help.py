        ...
        paginator = pages.Paginator(
            ...
            custom_buttons=[
                pages.PaginatorButton(
                    "first", label="<<-", style=discord.ButtonStyle.success
                ),
                pages.PaginatorButton(
                    "prev", label="<-", style=discord.ButtonStyle.success
                ),
                pages.PaginatorButton(
                    "page_indicator", style=discord.ButtonStyle.gray, disabled=True
                ),
                pages.PaginatorButton(
                    "next", label="->", style=discord.ButtonStyle.success
                ),
                pages.PaginatorButton(
                    "last", label="->>", style=discord.ButtonStyle.success
                ),
            ],
        )
    ...
    async def changelog(
        ...
        if version is not None:
            if version not in changelog:
                await ctx.respond(
                    f"I couldn't find v{version}, are you sure it exists?"
                )
            else:
                cl = changelog[version]
                embed = discord.Embed(
                    title=f"v{version} - {cl['title']} ({cl['update_type']})",
