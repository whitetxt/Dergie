...
class Reactions(commands.Cog):
    ...
    async def create_reaction_role(
        ...
        emoji: discord.Option(
            str, description="The emoji to use as the reaction", required=True
        ),
        message_content: discord.Option(
            str, description="Message content, leave blank to use existing message"
        ) = None,
    ):
        ...
        if message_content is None:
            await ctx.respond(
                "Please reply to the message, or send a link to the message to add a reaction role to."
            )
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        cur = db.cursor()
        result = cur.execute(
            "SELECT role_id FROM reaction_roles WHERE message_id = ? AND emoji_id = ?",
        ...
        if payload.member.get_role(role_id) is None:
            try:
                payload.member.add_roles(
                    role_id, reason=f"Reaction Role - Message ID {payload.message_id}"
                )
